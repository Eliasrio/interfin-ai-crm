...
6. **Токены Claude считаем гибридно.** Локальная оценка `len/4`; точный
   ...
   Бюджет system-блока `token_budget.system_prompt` = **5000** — замена:
   SRS §7.2 задавал 2000, поднято решением владельца 2026-07-11 (ТЗ панели
   Эммы §0/§10 п.6: секции панели — промпт ≤1200, контакты, файлы — плюс
   RAG-чанки в 2000 не влезали, база знаний вытеснялась; зафиксировано в
   EP-02). Потолок входа теперь 11000, запроса целиком — 12000; threshold
   7500 меньше возможного system-блока, точный count_tokens зовётся чаще —
   это ожидаемо.
