...
// BIGINT NOT NULL, уникальная по живым строкам (частичный индекс
// WHERE deleted_at IS NULL, миграция 0011); при erasure она НЕ NULL-ится,
// а заменяется хешем hash(user_id + salt). Требования к хешу:
//   - детерминированность при фиксированной соли (повторный erasure-запрос
//     не плодит новые значения); поэтому уникальность и НЕ распространяется
//     на стёртых: вернувшийся после erasure и снова стёртый лид даёт
//     вторую строку с тем же хешом — это корректно (один титуляр);
